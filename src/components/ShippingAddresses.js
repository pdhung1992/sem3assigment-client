...
const ShippingAddresses = () => {
    ...
    const fetchAddresses = async () => {
        const addData = await addressService.getShipAddByUser(user.userData.id, axiosConfig);
    ...
    const handleDelete = async (id) => {
        try {
            ...
            if (result.isConfirmed) {
                await addressService.deleteShipAddress(id, axiosConfig);
    ...
    const handleCreateSubmit = (e) => {
        ...
        const fetchNewAdd = async () => {
            try {
                const res = await addressService.createShipAddress(formData, axiosConfig);
                console.log(res)
                if(res.name){
    ...
    const showDetailModal = async (id) => {
        const detail = await addressService.getShipAddDetails(id, axiosConfig);
    ...
    const handleEditSubmit = (e) => {
        ...
        const fetchUpdateAdd = async () => {
            try {
                const res = await addressService.updateShipAddress(id, formData, axiosConfig);
                if(res){
    ...
    return(
        ...
        <div className={'db-content'}>
            <h3>Shipping Addresses Management:</h3>
            <hr/>
            <Row gutter={[16, 16]} justify="center">
                ...
                <Col xs={24} sm={24} md={24} lg={8} xl={8} xxl={8} style={{ textAlign: 'end' }}>
                    <button className={'main-btn btn'}   onClick={showCreateModal}>
                        <PlusOutlined /> Create New Shipping Address
                    </button>
                </Col>
            </Row>
            <hr/>
